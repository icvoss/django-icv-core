...
publish workflow, which tests, builds, uploads to PyPI, and creates a GitHub
release. **Tagging is publishing. There is no separate publish step and no
undo.**
...
2. **Bump** on the branch:
   - `pyproject.toml` at the repo root: update `version`
...
6. **Watch the publish run** and confirm PyPI:
   ...
   curl -s https://pypi.org/pypi/django-icv-core/json | python -c \
...
- `src/icv_core/__init__.py`: `__version__ = "0.3.0"`
...
- **Patch** (`0.3.0 -> 0.3.1`): bug fixes, doc-only changes, no API or
  behaviour change.
- **Minor** (`0.3.0 -> 0.4.0`): new public API, any behaviour change (even a
...
matching the version before proceeding.
...
The publish workflow test job pins `Django~=5.1.0`. When you raise the minimum
Django version in `pyproject.toml`, update the pin in the same PR or the
...
- [ ] Tag format is `v<version>` (plain semver, no package prefix).
